...
pub fn highlight_line(line: &str, ext: &str) -> String {
    match ext {
        "rs" => highlight_c_like(
            line,
            &[
                "fn", "let", "pub", "struct", "impl", "trait", "use", "mod", "match", "if",
                "else", "for", "in", "while", "return", "const", "static", "mut", "ref",
                "type", "enum", "as", "crate", "self", "Self", "where", "async", "await"
            ],
            &[
                "String", "Option", "Result", "u32", "i32", "u64", "i64", "usize", "isize",
                "str", "bool", "char", "f32", "f64"
            ],
        ),
        "js" | "ts" | "jsx" | "tsx" => highlight_c_like(
            ...
            &[
                ...
                "default", "return", "if", "else", "for", "in", "of", "while", "do",
                "switch", "case", "break", "continue", "new", "this", "super", "typeof",
                "instanceof", "async", "await", "yield", "interface", "type", "public",
                "private", "protected", "readonly", "any", "unknown", "never", "void"
            ],
        ...
        "go" => highlight_c_like(
            line,
            &[
                "func", "var", "const", "import", "package", "type", "struct", "interface",
                "map", "chan", "range", "go", "select", "defer", "if", "else", "for",
...
fn highlight_python_like(line: &str) -> String {
    let keywords = &[
        ...
        "and", "or", "not", "import", "from", "as", "try", "except", "finally",
        "raise", "assert", "with", "pass", "break", "continue", "global", "nonlocal",
...
pub fn show_viewer(path: &Path, rel_path: &str, extension: &str) -> Result<ViewResult> {
    ...
    let result = (|| -> Result<ViewResult> {
        loop {
            ...
            let term_height = if term_height == 0 { 24 } else { term_height as usize };
            let term_width = if term_width == 0 { 80 } else { term_width as usize };
            ...
            for idx in 0..display_lines {
                ...
                let truncated_line = if original_line.chars().count() > term_width {
                    let mut s: String = original_line.chars().take(term_width.saturating_sub(3)).collect();
                    s.push_str("...");
                    s
                } else {
